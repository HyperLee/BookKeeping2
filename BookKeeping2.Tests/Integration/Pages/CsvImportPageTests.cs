...
public sealed partial class CsvImportPageTests
{
    ...
    [Fact]
    public async Task English_mode_csv_import_localizes_non_error_summary_and_preserves_traditional_chinese_error_contract()
    {
        await using BookKeepingWebApplicationFactory factory = new();
        HttpClient client = factory.CreateClient(new() { AllowAutoRedirect = false });
        ...
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        Assert.Contains("CSV Import", body, StringComparison.Ordinal);
        Assert.Contains("Succeeded 1 rows, failed 1 rows", body, StringComparison.Ordinal);
        Assert.Contains("第 3 行匯入失敗", body, StringComparison.Ordinal);
