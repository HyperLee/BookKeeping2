...
- 所有非首頁的使用者可瀏覽 Razor Pages 不得顯示主題模式控制項。
- 所有目前可由使用者直接瀏覽的 Razor Pages 在 `light`、`dark`、`system` 模式下，手機、平板、桌面寬度不得有文字或控制項重疊。
- 主要內容、導覽、按鈕、連結、表單、表格、圖表、警示、驗證訊息與焦點狀態需符合 WCAG 2.2 AA。
